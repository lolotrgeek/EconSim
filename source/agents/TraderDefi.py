...
class TraderDefi():
    ...
    async def provide_liquidity(self, agent_wallet, base, quote, amount, fee_level=-1, high_range='.8', low_range='.2') -> dict:
        liquidity_provided = await self.exchange_requests.provide_liquidity(agent_wallet, base, quote, amount, fee_level, high_range, low_range)
        return liquidity_provided
    
    async def remove_liquidity(self, agent_wallet, base, quote, amount, fee_level=-1) -> dict:
        liquidity_removed = await self.exchange_requests.remove_liquidity(agent_wallet, base, quote, amount, fee_level)
        return liquidity_removed
    
    async def swap(self, agent_wallet, base, quote, amount, slippage='.05') -> dict:
        swap = await self.exchange_requests.swap(agent_wallet, base, quote, amount, slippage)
        return swap

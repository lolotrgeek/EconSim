...
class StockExchangeRequests(Requests):
    ...
    async def limit_buy(self, ticker,  price, quantity, creator):
        return await self.make_request('limit_buy', {'ticker' : ticker ,'price': price, 'qty': quantity, 'creator': creator}, self.requester) 

    async def limit_sell(self, ticker,  price, quantity, creator):
        return await self.make_request('limit_sell', {'ticker' : ticker, 'price': price, 'qty': quantity, 'creator': creator}, self.requester)
    ...
    async def market_buy(self, ticker,  quantity, creator):
        return await self.make_request('market_buy', {'ticker' : ticker, 'qty': quantity, 'buyer': creator}, self.requester)
    
    async def market_sell(self, ticker,  quantity, creator):
        return await self.make_request('market_sell', {'ticker' : ticker, 'qty': quantity, 'seller': creator}, self.requester)

...
from .types.Side import Side
from .types.Pair import Pair
from .types.Asset import Asset
from .types.FrozenAssets import FrozenAssets
from .types.Agent import Agent
from .types.Fee import Fee
from .types.Basis import Basis
from .types.Position import Position
from .types.Enter import Enter
from .types.Exit import Exit
from .types.TaxableEvent import TaxableEvent
...
class CryptoExchange(Exchange):
    def __init__(self, dt= None, requester=None, archiver=None):
        super().__init__(datetime=datetime)
        self.archiver = archiver
        self.requester = requester
        self.datetime: datetime = dt
        self.default_currency = Asset('fiat', 2, Decimal('0.01'), Decimal('0.05'), 'USD')
    ...
    async def list_asset(self, asset, pair, decimals, minimum, min_qty_percent):
        new_pairing = Pair(asset, pair['asset'], pair['ticker'], decimals, self.assets[pair['asset']].decimals, minimum, prec(minimum, self.assets[pair['asset']].decimals), min_qty_percent, True) 
    ...
    async def create_asset(self, symbol: str, pairs:list=[], decimals=8, min_qty_percent='0.05') -> dict:
        """_summary_
        Args:
        ...
        for pair in pairs:
            ...
            self.agents.append(Agent('init_seed_'+ticker, [quote_position], {}, wallets)), 
            self.assets[symbol] = Asset('crypto', decimals, minimum, Decimal(min_qty_percent))
            await self.list_asset(symbol, pair, decimals, minimum, Decimal(min_qty_percent))

        return {'asset_created': symbol, 'pairs': pairs}
        
    async def _process_trade(self, base, quote, qty, price, buyer, seller, seller_order_id, buyer_order_id, accounting='FIFO', exchange_fee=Fee('exchange',0,0), network_fee=Fee('network',0,0), position_id=None):
        try:
            ...
            transaction = [
                Side(buyer, buyer_order_id, -quote_amount, price, base, quote, qty, exchange_fee.quote, network_fee.quote, txn_time, 'buy'),
                Side(seller, seller_order_id, quote_amount, price, base, quote, qty, exchange_fee.base, network_fee.base, txn_time, 'sell')
            ]
    ...
    async def _complete_trade(self, transaction, base_transaction, quote_transaction):
        ...
        network_fee = Fee('network',prec(base_transaction.fee, self.assets[base].decimals), prec(quote_transaction.fee, self.assets[quote].decimals))
        exchange_fee = Fee('exchange',prec(transaction['exchange_txn'][1].fee,self.assets[base].decimals), prec(transaction['exchange_txn'][0].fee, self.assets[quote].decimals))
    ...
    async def freeze_assets(self, agent: str, asset, order_id, qty=0, exchange_fee=0, network_fee=0) -> dict:
        """
        ...
        if asset not in self.agents[agent_idx].frozen_assets:
            ...
            for existing_frozen_assets in self.agents[agent_idx].frozen_assets[asset]:
                existing_frozen_assets: FrozenAssets
    ...
    async def unfreeze_assets(self, agent: str, asset: str, order_id: str, qty=0, exchange_fee=0, network_fee=0) -> dict:
        agent_idx = await self.get_agent_index(agent)
        if agent_idx is None:
    ...
    async def register_agent(self, name: str, initial_assets={}) -> dict:
        """
        ...
        positions = []
        wallets = {self.default_currency.symbol: (await self.generate_address())}
        for asset in initial_assets:
            qty = prec(initial_assets[asset], self.assets[asset].decimals)
            fee = Fee('seed', 0, 0)
            side = Side(registered_name, 'seed', 0, 0, asset, self.default_currency.symbol, qty, fee, fee, self.datetime, 'buy')
            basis = Basis(self.default_currency.symbol, 0, 'seed', self.datetime)
        ...
        self.agents.append(Agent(registered_name, positions, initial_assets, wallets))
        self.logger.debug('registered agent', self.agents[-1])
        return {'registered_agent':registered_name}
    
    async def new_position(self, side: Side, asset, qty, basis={}) -> Position:
        qty = prec(qty, self.assets[asset].decimals)
        enter = Enter(side.agent, asset, qty, side.dt, side.type, basis)
        return Position(asset, qty, side.dt, [enter], [])

    async def taxable_event(self, agent: Agent, amount: Decimal, dt: datetime, basis_amount, basis_date) -> None:
        self.logger.info('taxable event', amount, basis_amount)
        pnl = amount - basis_amount
        self.logger.info('pnl', pnl)
        if pnl > 0:
            taxable_event = TaxableEvent('capital_gains', basis_date, dt, pnl)
            agent.taxable_events.append(taxable_event)        

    async def enter_position(self, side: Side, asset, qty, agent_idx, position_id, basis={}) -> Enter:
        ...
        if start_new_position:
            ...
            positions.append(new_position) 
            return new_position.enters[0]
        
    async def exit_position(self, side: Side, asset: str, qty: Decimal, agent_idx: int) -> Exit:
        exit_transaction: Side = side.copy()
        agent: Agent = self.agents[agent_idx]
        positions: list = agent.positions
        ...
        while exit_transaction.qty > 0:
            ...
            for position in positions:
                ...
                enters: List[Enter]
                for enter in enters:
                    exit = Exit(agent.name, asset, exit_transaction.dt, enter.id)
                    ...
                    if side.quote == self.default_currency.symbol and asset == self.default_currency.symbol:
                        ...
                        exit.basis = Basis(side.base, basis_total, exit_transaction.id, exit.dt) #NOTE this exit_transaction id is stored here -> self.agents[agent_idx]._transactions
                        exit.basis.basis_per_unit = prec(side.price, self.assets[side.quote].decimals)
                    
                        if enter.basis.basis_txn_id == 'seed': enter.basis = exit.basis.copy()
            ...
            exit.error = 'no positions to exit'
            return exit     
        self.logger.error(f"unable to find viable exit {side.agent} {side.order_id} {asset} {qty} {side.type}")
        exit = Exit(agent.name, asset, exit_transaction.dt, "null_exit")
        exit.error = 'unable to find viable exit'        
        return exit                           
    ...
    async def update_agents(self, transaction, accounting, position_id) -> None:
        for side in transaction:
            ...
            if side.type == 'buy':
                ...
                enter = await self.enter_position(side, side.base, side.qty, agent_idx, position_id, basis=exit.basis)
                ...
                if exit.error:
                    return {'update_agents': 'unable to exit position'}
                enter = await self.enter_position(side, side.quote, side.quote_flow, agent_idx, None, basis=exit.basis)
            else:
    ...
    async def get_agent(self, agent_name)  -> Agent:

...
async def run_crypto_exchange() -> None:
    try:
        ...
        async def callback(msg) -> str:
            if msg['topic'] == 'create_asset': return dumps((await exchange.create_asset(msg['symbol'], msg['pairs'], msg['decimals'], msg['min_qty_percent'])))
            elif msg['topic'] == 'sim_time': return dumps(exchange.datetime)
            elif msg['topic'] == 'get_tickers': return dumps((await exchange.get_tickers()))
            elif msg['topic'] == 'limit_buy': return dumps((await exchange.limit_buy(msg['base'] , msg['quote'], msg['price'], msg['qty'], msg['creator'], msg['fee'], msg['min_qty'])).to_dict_full())
            elif msg['topic'] == 'limit_sell': return dumps((await exchange.limit_sell(msg['base'] , msg['quote'], msg['price'], msg['qty'], msg['creator'], msg['fee'], msg['min_qty'])).to_dict_full())
            elif msg['topic'] == 'market_buy': return dumps((await exchange.market_buy(msg['base'] , msg['quote'], msg['qty'], msg['buyer'], msg['fee'])).to_dict_full())
            elif msg['topic'] == 'market_sell': return dumps((await exchange.market_sell(msg['base'] , msg['quote'], msg['qty'], msg['seller'], msg['fee'])).to_dict_full())
            ...
            elif msg['topic'] == 'get_pending_transactions': return dumps(await exchange.get_pending_transactions(msg['limit']))

...
class FreezeAssetsTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_freeze_asset(self):
        await self.exchange.freeze_assets(self.agent, "BTC", "test_order", 100, Decimal('0.01'))
    ...
    async def test_freeze_no_asset(self):
        freeze = await self.exchange.freeze_assets(self.agent, "ETH", "test_order", 100, Decimal('0.01'))
        agent = await self.exchange.get_agent(self.agent)
    ...
    async def test_freeze_insufficient_assets(self):
        freeze = await self.exchange.freeze_assets(self.agent, "BTC", "test_order", 100000, Decimal('0.01'))
        agent = await self.exchange.get_agent(self.agent)
...
class LimitOrderMatchingTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_limit_order_match_trades(self):
        ...
        trade_payment_USD = prec(new_order.fills[0]['qty'] * new_order.fills[0]['price'] + new_order.fills[0]['fee'])
        self.assertEqual(agent['assets'], {'BTC': Decimal('10004.000000000000000000'), 'USD': prec(10000 - trade_payment_USD)})
        trade_BTC_earned = new_order.fills[0]['qty'] + self.exchange.trade_log[1].qty + self.exchange.trade_log[1].exchange_fee['base'] + self.exchange.trade_log[2].exchange_fee['base']
        ...
        BTC_in_trade = self.exchange.trade_log[1].qty + self.exchange.fees.fees_collected['BTC'] + agent['assets']['BTC'] + agent_seller['assets']['BTC'] + agent_seller['frozen_assets']['BTC'][0]['frozen_qty'] + agent_seller['frozen_assets']['BTC'][0]['frozen_exchange_fee']
        ...
        self.assertEqual(BTC_in_trade, Decimal('20000'))
        self.assertEqual(USD_in_trade, Decimal('20000') +USD_from_init)

    async def test_limit_sell_partial_match(self):
        # half of the order gets filled, the other half becomes a maker order
        sell_qty = 10
        sell_order = await self.exchange.limit_sell("BTC", "USD", price=145, qty=sell_qty, creator=self.seller)
        new_order = await self.exchange.limit_buy('BTC', "USD", 152, 5, self.buyer)
        agent = await self.exchange.get_agent(self.buyer)
        agent_seller = await self.exchange.get_agent(self.seller)
        self.assertEqual(new_order.ticker, 'BTCUSD')
        ...
        self.assertEqual(new_order.creator, self.buyer)
        trade_payment_USD = prec(new_order.fills[0]['qty'] * new_order.fills[0]['price'] + new_order.fills[0]['fee'], 2)
        self.assertEqual(agent['assets'], {
            'BTC': Decimal('10005'),
            'USD': Decimal('10000') - trade_payment_USD
        })
        trade_BTC_earned = self.exchange.trade_log[1].qty +self.exchange.trade_log[2].qty + self.exchange.trade_log[1].exchange_fee['base']+ self.exchange.trade_log[2].exchange_fee['base']
        self.assertEqual(agent_seller['assets'], {
            'BTC': 10000 - trade_BTC_earned - agent_seller['frozen_assets']['BTC'][0]['frozen_qty'] - agent_seller['frozen_assets']['BTC'][0]['frozen_exchange_fee'],
        ...
        self.assertEqual(agent_seller['frozen_assets']['BTC'][0]['frozen_exchange_fee'], self.exchange.books['BTCUSD'].asks[0].exchange_fee)
         #NOTE: we add 1 because the sell order fills the init_seed bid
        #NOTE: except for the network fees, all the funds in the above trade should still be in the exchange, just in different accounts
        BTC_in_trade = self.exchange.trade_log[1].qty + self.exchange.fees.fees_collected['BTC'] + agent['assets']['BTC'] + agent_seller['assets']['BTC'] + agent_seller['frozen_assets']['BTC'][0]['frozen_qty'] + agent_seller['frozen_assets']['BTC'][0]['frozen_exchange_fee']
        USD_in_trade = self.exchange.fees.fees_collected['USD'] + agent['assets']['USD'] + agent_seller['assets']['USD']
        USD_from_init = self.exchange.trade_log[1].qty*self.exchange.trade_log[1].price + self.exchange.trade_log[1].exchange_fee['quote'] #NOTE: we have to include this because the seller receives monies from the init_seed bid
    ...
    async def test_limit_buy_partial_match(self):
        ...
        trade_payment_USD = prec(self.exchange.trade_log[2].qty * self.exchange.trade_log[2].price + self.exchange.trade_log[2].exchange_fee['quote'], 2)
        self.assertEqual(agent['assets'], {
            'BTC': Decimal('10004'),
            'USD': Decimal('10000') - trade_payment_USD - agent['frozen_assets']['USD'][0]['frozen_qty'] - agent['frozen_assets']['USD'][0]['frozen_exchange_fee']
        })
        #NOTE: BTC going to be 10004 because 1 is sold to the init_seed bid
        trade_BTC_earned = new_order.fills[0]['qty'] + new_order.fills[0]['fee'] + new_order.fills[1]['qty'] + new_order.fills[1]['fee']
        ...
        #NOTE: except for the network fees, all the funds in the above trade should still be in the exchange, just in different accounts
        USD_in_trade =  + self.exchange.fees.fees_collected['USD'] + agent['assets']['USD'] + seller['assets']['USD'] + agent['frozen_assets']['USD'][0]['frozen_qty'] + agent['frozen_assets']['USD'][0]['frozen_exchange_fee']
...
class LimitOrderMarketMakingTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_market_make_complete(self):
        ...
        self.assertEqual(new_order.qty, 5)
        self.assertEqual(new_order.creator, self.buyer)
        new_order_payment_USD = new_order.fills[0]['qty'] * new_order.fills[0]['price'] + new_order.fills[0]['fee']
        complete_order_payment_USD = complete_order.fills[0]['qty'] * complete_order.fills[0]['price'] + complete_order.fills[0]['fee']
        self.assertEqual(agent['assets'], {
            'BTC': Decimal('10010'),
            'USD': Decimal('10000') - new_order_payment_USD - complete_order_payment_USD
        })
        new_order_BTC_earned = self.exchange.trade_log[1].qty +self.exchange.trade_log[2].qty + self.exchange.trade_log[1].exchange_fee['base'] + self.exchange.trade_log[2].exchange_fee['base']
        complete_order_BTC_earned = self.exchange.trade_log[3].qty+ self.exchange.trade_log[3].exchange_fee['base']
        self.assertEqual(agent_seller['assets'], {
...
class MarketBuyTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_market_buy(self):
        ...
        self.assertEqual(agent['assets'], {'BTC': Decimal('4'), 'USD': Decimal('499392.79')})
    ...
    async def test_no_fills(self):
        ...
        self.assertEqual(len(agent['frozen_assets']['USD']), 1 )
...
class MarketSellTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_market_sell(self):
        ...
        BTC_sold = prec(self.exchange.trade_log[1].qty + self.exchange.trade_log[2].qty + self.exchange.trade_log[1].exchange_fee['base'] + self.exchange.trade_log[2].exchange_fee['base'])
        self.assertEqual(agent['assets'], {"BTC": 500000-BTC_sold, "USD": USD_from_trade})
        USD_To_Buy = prec(self.exchange.trade_log[2].qty*self.exchange.trade_log[2].price + self.exchange.trade_log[2].exchange_fee['quote'])
        USD_Buy_Frozen = prec(buyer_agent['frozen_assets']['USD'][0]['frozen_qty'] + buyer_agent['frozen_assets']['USD'][0]['frozen_exchange_fee'])
...
class MarketMatchingTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_market_partial_matching(self):
        ...
        cash_from_trade = result.fills[0]['qty'] * result.fills[0]['price'] + exchange_fee
    ...
    async def test_buy_multi_fill(self):
        ...
        cash_from_trade = result.fills[0]['qty'] * result.fills[0]['price'] +result.fills[1]['qty'] * result.fills[1]['price'] + exchange_fee
...
class FractionalOrdersTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fractional_buy(self):
        ...
        agent = await self.exchange.get_agent(self.buyer1)
        self.assertEqual(agent['assets'], {"ETH": Decimal('0.00005'), "BTC":Decimal('499999.99997470')} )
        self.assertEqual(agent['frozen_assets']['BTC'][0]['order_id'], new_order.id)
    ...
    async def test_fractional_sell(self):
        ...
        agent = await self.exchange.get_agent(self.seller1)
        buyer_agent = await self.exchange.get_agent(self.buyer1)
        ETH_traded = prec(self.exchange.trade_log[3].qty + self.exchange.trade_log[3].exchange_fee['base'], 18)
    ...
    async def test_fractional_price(self):
        ...
        trade_1 = prec(self.exchange.trade_log[5].qty*self.exchange.trade_log[5].price + self.exchange.trade_log[5].exchange_fee['quote'], 8)
        trade_2 = prec(self.exchange.trade_log[6].qty*self.exchange.trade_log[6].price + self.exchange.trade_log[6].exchange_fee['quote'], 8)
        BTC_from_trade = prec(buy_single.fills[0]['qty']*buy_single.fills[0]['price'] + buy_single.fills[0]['fee'] + buy_fraction.fills[0]['qty']*buy_fraction.fills[0]['price'] + buy_fraction.fills[0]['fee'] , 8)
    ...
    async def test_large_fractional_qty(self):
        generous = await self.exchange.market_buy("ETH", "BTC", qty=1000, buyer=self.generous)
        market_made= await self.exchange.limit_sell("ETH", "BTC", price='0.00005', qty=996, creator=self.generous, min_qty='0.000000000001234')
        buy_single = await self.exchange.limit_buy("ETH", "BTC", price='0.00015', qty=1, creator=self.buyer1, fee='0.01')
        ...
        self.assertEqual(books.asks[0].price, Decimal('0.00005'))
        ETH_traded = prec(self.exchange.trade_log[5].qty + self.exchange.trade_log[6].qty, 18)
        BTC_from_trade = prec(self.exchange.trade_log[5].qty*self.exchange.trade_log[5].price + self.exchange.trade_log[5].exchange_fee['quote']  + self.exchange.trade_log[6].qty*self.exchange.trade_log[6].price + self.exchange.trade_log[6].exchange_fee['quote'], 8)
...
class GetTradesTestCase(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        await self.exchange.limit_sell("BTC", "USD", price=152, qty=2, creator=self.trader2, fee='0.02000002')
...
class CancelOrderTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cancel_partial_order(self):
        ...
        self.assertEqual(agent['frozen_assets']['USD'][0]['frozen_exchange_fee'], Decimal('0.00000'))
        self.assertEqual(agent['frozen_assets']['USD'][0]['order_id'], buy_order.id)
        trade_payment_USD = self.exchange.trade_log[2].qty * self.exchange.trade_log[2].price + self.exchange.trade_log[2].exchange_fee['quote']
        self.assertEqual(agent['assets'], {
            'BTC': Decimal('10004'),
            'USD': Decimal('10000') - trade_payment_USD - agent['frozen_assets']['USD'][0]['frozen_qty'] - agent['frozen_assets']['USD'][0]['frozen_exchange_fee']
        })
...
class GetAssetsTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_assets(self):
        ...
        self.assertEqual(result['assets']['USD'], Decimal('9696.40'))
        self.assertEqual(agent['assets'], {"BTC": Decimal('2'), "USD": Decimal('9696.40')})
        self.assertEqual(agent['frozen_assets']['USD'][0]['frozen_qty'], Decimal('0.00'))
        self.assertEqual(agent['frozen_assets']['USD'][0]['frozen_exchange_fee'], Decimal('0.00000'))
        self.assertEqual(agent['frozen_assets']['USD'][0]['order_id'], order.id)
...
class FreezeAssetTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_freeze_asset(self):
        await self.exchange.freeze_assets(self.agent, "USD", "test_id", 1000, prec('0.0001'))
        agent = await self.exchange.get_agent(self.agent)
...
class UnFreezeAssetTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_unfreeze_asset(self):
        await self.exchange.freeze_assets(self.agent, "USD", "test_id", 1000, prec('0.0001'))
        await self.exchange.unfreeze_assets(self.agent, "USD", "test_id", 1000, prec('0.0001'))
        agent = await self.exchange.get_agent(self.agent)
        self.assertEqual(agent['frozen_assets']['USD'][0]['frozen_exchange_fee'], 0)
...
class getAgentsCashTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_agents_cash(self):
        ...
        self.assertEqual(result[1][self.agent]['cash'], Decimal('9696.40'))
...
class totalCashTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_total_cash(self):
        ...
        self.assertEqual(result, Decimal('9696.40'))
...
class UpdateAgentsTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_update_agents(self):
        ...
        await self.exchange.freeze_assets(self.agent1, "USD", transaction[0]['order_id'], 50, Decimal('0.01'))
        await self.exchange.freeze_assets(self.agent2, "BTC", transaction[1]['order_id'], 1, Decimal('0.01'))

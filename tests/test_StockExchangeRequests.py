...
import unittest
from source.exchange.StockExchangeRequests import StockExchangeRequests as Requests
from .MockRequesterStock import MockRequester
...
class LimitBuyTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_limit_buy(self):
        response = await self.requests.make_request('limit_buy', {'ticker': "AAPL", 'price': 149, 'qty': 2, 'creator': self.mock_requester.responder.agent}, self.mock_requester)
        order = response
        ...
        self.assertEqual(order['fee'], '0.298')
...
class LimitSellTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_limit_sell(self):
        response = await self.requests.make_request('limit_sell', {'ticker': "AAPL", 'price': 151.5, 'qty': 1, 'creator': self.mock_requester.responder.agent}, self.mock_requester)
        order = response
        print(order)
        self.assertEqual(order['ticker'], "AAPL")
        ...
        self.assertEqual(order['creator'], self.mock_requester.responder.agent)
        self.assertEqual(order['fee'], '0.1515')
...
class CancelAllOrdersTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cancel_all_orders(self):
        ...
        self.assertEqual(response, {'cancelled_orders': [self.mock_requester.responder.mock_order.to_dict()['id']]})
...
class GetCashTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_cash(self):
        ...
        self.assertEqual(response, {'cash': Decimal('99850.851')})
...
class GetAssetsTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_assets(self):
        ...
        self.assertEqual(response, {'assets': {'AAPL': Decimal('0'), 'USD': Decimal('149699.8515')}, 'frozen_assets': {'AAPL': Decimal('1000'), 'USD': Decimal('300.1485')}})
...
class MarketBuyTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_market_buy(self):
        response = await self.requests.make_request('market_buy', {'ticker': 'AAPL', 'qty': 1, 'buyer': self.mock_requester.responder.agent}, self.mock_requester)
        print(response)
        self.assertEqual(response, {'market_buy': 'AAPL', 'buyer': self.mock_requester.responder.agent, 'qty': Decimal('1'), 'fills': [{'qty': Decimal('1'), 'price': Decimal('151.5'), 'fee': Decimal('0.3030')}]})
...
class MarketSellTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_market_sell(self):
        response = await self.requests.make_request('market_sell', {'ticker': 'AAPL', 'qty': 1, 'seller': self.mock_requester.responder.agent}, self.mock_requester)
        self.assertEqual(response, {'market_sell': 'AAPL', 'seller': self.mock_requester.responder.agent, 'qty': Decimal('1'), 'fills': [{'qty': Decimal('1'), 'price': Decimal('148.5'), 'fee': Decimal('0.2970')}]})
...
class GetAgentTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_agent(self):
        ...
        self.assertEqual(response['assets'], {'AAPL': '100', 'USD': '99850.851'})
        ...
        self.assertEqual(response['positions'][0]['enters'][0]['qty'], '100000')
        self.assertEqual(response['positions'][0]['enters'][0]['asset'], "USD")
        self.assertEqual(response['positions'][0]['exits'], [])
        self.assertEqual(response['positions'][0]['dt'], '2023-01-01 00:00:00')
        self.assertEqual(response['taxable_events'], [])
...
class GetAgentsTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_agents(self):
        ...
        self.assertEqual(response[1]['name'], self.mock_requester.responder.agent)
        self.assertEqual(response[1]['_transactions'], [])
        self.assertEqual(response[1]['positions'][0]['dt'], '2023-01-01 00:00:00')
        self.assertEqual(len(response[1]['positions'][0]['enters']), 1)
        self.assertEqual(response[1]['positions'][0]['exits'], [])
        self.assertEqual(response[1]['positions'][0]['qty'], '100000')
        self.assertEqual(response[1]['positions'][0]['asset'], 'USD')
        self.assertEqual(response[1]['assets'], {'AAPL': '100', 'USD': '99850.851'})
        self.assertEqual(response[1]['taxable_events'], [])
...
class AddCashTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_add_cash(self):
        ...
        self.assertEqual(response, {'USD': '100850.851'})
...
class RemoveCashTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_remove_cash(self):
        ...
        self.assertEqual(response, {'USD': '98850.851'})
...
class GetAgentsHoldingTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_agents_holding(self):
        ...
        self.assertEqual(response, ['init_seed_AAPL', self.mock_requester.responder.agent])
...
class GetAgentsPositionsTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_agents_positions(self):
        ...
        self.assertEqual(result[0]['positions'][0]['asset'], 'AAPL')
...
class GetAgentSimpleTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_agent_simple(self):
        ...
        self.assertEqual(result[0], {'agent': 'init_seed_AAPL', 'assets': {'AAPL': '0', 'USD': '149699.8515'}, 'frozen_assets': {'USD': '300.1485', 'AAPL': '1000'}})
        self.assertEqual(result[1], {'agent': self.mock_requester.responder.agent, 'assets': {'AAPL': '100', 'USD': '99850.851'}, 'frozen_assets': {'USD': '149.149'}})
...
class GetPositionsTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_positions(self):
        ...
        self.assertEqual(result['positions'][1]['qty'], '100')
